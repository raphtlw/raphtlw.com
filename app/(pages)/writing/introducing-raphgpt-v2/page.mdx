...
<VideoPlayer src="https://cdn.sanity.io/media-libraries/ml7E2FKaGXGN/files/8ac410f1b2927d0c478684dd227f99130fedf2d5.mp4" />
...
<Image src="https://cdn.sanity.io/media-libraries/ml7E2FKaGXGN/images/42ea2a907911cdeae7d91625bdcf24c5e9748d83-394x795.png" alt="Screenshot" width={394} height={795} />
...
<Image src="https://cdn.sanity.io/media-libraries/ml7E2FKaGXGN/images/5a697e9cdcfa967cfaf851be684419cdd95f28b9-3544x2234.png" alt="Transformers GPT-2 web interface" />
...
![Multimodal Architecture Diagram](https://miro.medium.com/v2/resize:fit:1400/1*-goPY__Yp_SRWiDivANqsA.png)

> From [Multimodal Large Language Models (MLLMs) transforming Computer Vision](https://medium.com/@tenyks_blogger/multimodal-large-language-models-mllms-transforming-computer-vision-76d3c5dd267f)
...
As [@sdw, the creator of halide camera puts it](https://x.com/sdw/status/1703872763125981414), Zero Shutter Lag lets you take photos—before they are taken.

![Zero Shutter Lag on Halide](https://pbs.twimg.com/media/F6Va3C6XQAAiEx4?format=jpg&name=small)
...
![Frame buffer visualisation](https://www.nxrte.com/wp-content/uploads/2022/11/640-25.png)

> Taken from [图像处理 &bull; 实时互动网](https://www.nxrte.com/jishu/5020.html)
...
![Computer vision explained](https://cvexplained.wordpress.com/wp-content/uploads/2020/08/image-20.png)

> Taken from [cvexplained.wordpress.com](https://cvexplained.wordpress.com/2020/08/16/blur-detection/)
...
![HDR visualisation](https://www.sony-semicon.com/files/62/t-22_6_HDR_04_en.png)

> Taken from [High Dynamic Range (HDR) Technology for Mobile](https://www.sony-semicon.com/en/technology/mobile/hdr.html)
...
![Image signal processing pipeline](https://10xengineers.ai/wp-content/uploads/Group-38295-min.png)

> Taken from [Image Signal Processing](https://10xengineers.ai/image-signal-processing/)
...
![Image signal processing pipeline](https://github.com/cruxopen/openISP/raw/master/images/Image%20Signal%20Processor.png)

> Taken from [Open Image Signal Processor (openISP)](https://github.com/cruxopen/openISP)
...
<div className='w-40'>
<img src="https://cdn.sanity.io/media-libraries/ml7E2FKaGXGN/images/7c23b517b601ccd185bb1a70c798fc45a7384643-606x2038.png" alt="raphGPT File structure" />
</div>
...
<details>
<summary>This was the Dockerfile from V1</summary>
...
<details>
<summary>Additional information about V1</summary>
...
1. Telegram bots are notorious for privacy concerns. I wanted to put privacy first, so I implemented a /clear command, which wipes the database completely of your messages and I will never be able to see it.
2. Media groups &mdash;you can send groups of photos and the bot would take it as one big message.
3. Interruptions. You can keep sending more and more messages, and as long as the bot didn't finish what it was about to say, it will start over, taking the newer messages into account.
4. Places and Navigation tools allow you to quickly find a place, find restaurants near you, look up the location of the most popular place in town, and even get directions to that place, right from within the bot.
5. Message scheduling. You can now schedule messages for sending later.
6. LTA DataMall API. It now has connectivity to LTA's data APIs which (if you live in Singapore), you can benefit from the vast availability of transportation devices and methods. The bot can now tell you:
...
7. Failure and error handling. If a message fails to process for any technical reasons due to connectivity, the bot keeps the message in redis and refers back to it when you send another message, telling it to retry.
8. Solana blockchain integration. You can look up any wallet by address or .sol Bonafide address and have it tell you things like:
...
9. [agentic.so](https://agentic.so) integration, which consists various LLM tools that are useful for general questions
10. Codex CLI integration. It's also able to operate on a zip file you send in to the bot, work on your project per your instructions, and send it back. It's really cool and I'd love for you to try it out.
...
Did I also mention _**agents that call other agents**_? They are entities that keep track of their own context memory, can call other agents, and answers back to the main agent.

![Diagram that illustrates the agent architecture in this bot](https://cdn.sanity.io/media-libraries/ml7E2FKaGXGN/images/ae3e662339ca7ede36493f4badcf2b8f9bda4488-1296x778.png)

Imagine a magical little café run entirely by cats. But these aren’t ordinary cats — each one is a specialist, trained to help you with specific tasks. This café is your computer, and these cats are your AI agents.

🧁 1. Chef Whiskers – the Task Agent

Chef Whiskers is great at baking the perfect cake. Give her a recipe or a goal (“make a chocolate soufflé”), and she’ll get straight to work, step by step, until your dessert is done.

Like an AI agent executing a to-do list or solving a problem end-to-end.

☕ 2. Barista Mittens – the Planning Agent

Mittens doesn’t do the baking, but she’s a genius at organizing. If you ask her, “How can I plan a birthday party?”, she’ll gather the shopping list, schedule the timing, and assign tasks to the others.

Like a planner agent who breaks down goals into smaller actions.

📦 3. Runner Paws – the Action Agent

Paws is fast. You tell her to deliver milk to Chef Whiskers or fetch more catnip tea, and she zips off and gets it done.

Like an agent that interacts with the outside world or APIs.

🧠 4. Professor Meowstein – the Memory Agent

Old and wise, Meowstein remembers every customer who came in, what they liked, and where they sat. Ask her anything about the past and she’ll give you a purrcise answer.

Like an agent storing and recalling long-term memory for better context.

🎤 5. Host Luna – the Communication Agent

Elegant and chatty, Luna greets every visitor, listens to their requests, and decides which cat should help. She’s the bridge between customers and the rest of the team.

Like a front-end conversational agent that delegates to others.

### How They Work Together

Let’s say a customer walks in and says:

“I want a customized birthday cupcake, with a card, and a delivery tonight.”

	1.	Luna hears the request and alerts Mittens.
	2.	Mittens creates a plan: Bake, write card, schedule delivery.
	3.	Chef Whiskers bakes the cupcake.
	4.	Runner Paws delivers it.
	5.	Meowstein logs the order for future loyalty rewards.

They all work together, quietly collaborating behind the scenes. You, the human, just sit back and enjoy your tea. 🍵

![](https://cdn.sanity.io/media-libraries/ml7E2FKaGXGN/images/7b5427cfba9774c75ba91396736e7fa3a6acabb2-1365x1024.webp)

These are now included in the architecture located under bot/agents.ts. It's a very cool concept, because not only do these AI models have the ability to call tools, they can also run other agents. For example, wouldn't it be nice if an excel file modification agent could run say, the schedule_message tool? This would allow the model to schedule messages to other "contacts" based on the contents of the excel spreadsheet. For instance, you want it to go over the guest list for a birthday party and send each one of them a custom email.
...
<TappableLink href="https://t.me/raphgptbot" icon={<LinkIcon size={16} />}>
   <span>Try it today on Telegram</span>
</TappableLink>
