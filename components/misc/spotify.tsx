...
export const getSpotifyNowPlaying = async () => {
  ...
  if (!nowPlayingResponse.ok) {
    console.error(
      ...
      await nowPlayingResponse.text(),
    );
    throw new Error("Failed to get now playing from Spotify");
  }
  return await nowPlayingResponse.json();
};
  ...
  try {
    ...
    return (
      <div className="flex flex-row gap-8">
        ...
        <ExternalLink href={nowPlaying.item.external_urls.spotify} icon>
          {`${nowPlaying.item.name} — ${nowPlaying.item.artists[0].name}`}
        </ExternalLink>
      </div>
    );
  } catch {
